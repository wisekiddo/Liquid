...
public class RemoteDataSource implements DataSource {
    ...
    private static final Map<Integer, Photo> mapPhotos;
    ...
    public void savePhoto(@NonNull Photo photo) {
        mapPhotos.put(photo.getId(), photo);
    }
    ...
    @Override
    public Flowable<List<Photo>> getPhotos(Integer id) {
        service = ApiClient.getClient().create(ApiService.class);
        return  service.getPhotos(id)
                .flatMap(photos -> Flowable.fromIterable(photos).doOnNext(photo -> {
                    addPhoto(photo);
                }).toList().toFlowable());
    }
    ...
    public Flowable<Photo> getPhoto(@NonNull Integer id) {
        final Photo photo = mapPhotos.get(id);
        if (photo != null) {
